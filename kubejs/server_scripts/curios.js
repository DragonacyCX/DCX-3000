...
PlayerEvents.loggedIn( event => {
    ...
    if (!event.player.stages.has('curios_slots_added')) {
      ...
      event.server.runCommandSilent("curios add belt 1" + player_name)   //server runs a command that adds an aditional belt slot to the new player
      event.server.runCommandSilent("curios add ring 1" + player_name)   //server runs a command that adds an aditional ring slot to the new player
      event.server.runCommandSilent("curios add head 1" + player_name)
      event.server.runCommandSilent("curios add back 1" + player_name)
